...
int n=0, m=0;      //n表示集合A中的元素个数，m表示二元关系R中的元素个数 
...
void Warshall(char*& A, BR*& R, bool**& tR)
{
	...
	for (i = 0; i < m; i++)
	{
		x = fun(R[i].a, A);// 找到字符在集合中的下标
		y = fun(R[i].b, A);

		// 防止非法输入导致越界
		if (x == -1 || y == -1) {
			cout << "警告：关系 <" << R[i].a << "," << R[i].b << "> 中含有不在集合A中的元素，已跳过。" << endl;
			continue;
		}
	...
	{ //检索列 
		for (j = 0; j < n; j++) //起始节点
		{ //检索行 
			if (tR[j][i] == 1)  // 如果 j -> i 存在路径
			{
				for (k = 0; k < n; k++)
				{
					tR[j][k] = tR[j][k] || tR[i][k];// 更新：j -> k 或 j -> i -> k
				}
			}
		}
	}
